...
if __name__ == '__main__':
    ...
    parser.add_argument('--body_asso', type=int, default=0, help='whether using body association or not')
    ...
    assert data['dataset'] == "CrowdHuman", "We now only support models trained on CrowdHuman!!!"
    ...
    for index in range(len(dataset)):
        ...
        if args.body_asso:  # using body_association
        ...
        else:  # not using body_association
            if len(parts_dict) != 0:
        ...
        cv2.imwrite(single_path[:-4]+"_res_head_%s.jpg"%(args.body_asso), im0)
