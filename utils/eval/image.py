...
class Image(object):
    ...
    def load(self, record, body_key, head_key, class_names, gtflag, if_face=False, body_part='body'):
        """
        :meth: read the object from a dict
        """
        if "ID" in record and self.ID is None:
            # self.ID = record['ID']
            self.ID = record['ID'][0] if isinstance(record['ID'], list) else record['ID']
        if "width" in record and self._width is None:
            # self._width = record["width"]
            self._width = record['width'][0] if isinstance(record['width'], list) else record['width']
        if "height" in record and self._height is None:
            # self._height = record["height"]
            self._height = record['height'][0] if isinstance(record['height'], list) else record['height']
        if gtflag:
            # self._gtNum = len(record["gtboxes"])
            body_bbox, head_bbox = self.load_gt_boxes(record, 'gtboxes', class_names, if_face, body_part)
    ...
    def load_gt_boxes(self, dict_input, key_name, class_names, if_face=False, body_part='body'):
        ...
        for rb in dict_input[key_name]:
            if rb['tag'] in class_names:  # all 'tag' is 'person' in GT json file
            ...
            if not if_face:
                body_bbox.append(np.hstack((rb['bbox'], body_tag)))  # for body bbox loading
            else:
                if body_part == "face":  # for body-part (face) bbox loading
                    if rb['fbox'][2] > 1:
                        if body_tag == -1:  # body-part is legal, but body is illegal
                            tag_f = -1
                            body_bbox.append(np.hstack((rb['bbox'], tag_f)))
                            # body_bbox.append(np.hstack((rb['fbox'], tag_f)))
                        else:  # body-part is legal, and body is illegal
                            tag_f = 2
                            body_bbox.append(np.hstack((rb['fbox'], tag_f)))
                    else:
                        if body_tag == -1:  # body-part is illegal, and body is illegal
                            body_bbox.append(np.hstack((rb['bbox'], -1)))
                            # body_bbox.append(np.hstack((rb['fbox'], -1)))
                            
                if body_part == "head":  # for body-part (head) bbox loading
                    if rb['hbox'][2] > 1:
                        if body_tag == -1:  # body-part is legal, but body is illegal
                            tag_f = -1
                            body_bbox.append(np.hstack((rb['bbox'], tag_f)))
                            # body_bbox.append(np.hstack((rb['hbox'], tag_f)))
                        else:  # body-part is legal, and body is illegal
                            tag_f = 2
                            body_bbox.append(np.hstack((rb['hbox'], tag_f)))
                    else:
                        if body_tag == -1:  # body-part is illegal, and body is illegal
                            body_bbox.append(np.hstack((rb['bbox'], -1)))
                            # body_bbox.append(np.hstack((rb['hbox'], -1)))
        ...
        if len(body_bbox) < 1:
            ...
            body_bbox[:, 2:4] += body_bbox[:, :2]  # [x,y,w,h] --> [x1,y1,x2,y2]
