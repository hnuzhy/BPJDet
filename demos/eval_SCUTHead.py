...
if __name__ == '__main__':
    ...
    # root_path: /datasdc/zhouhuayi/dataset/HeadDetection/
    # SCUT_HEAD_Part_A/B --> Annotations: xxxx.xml
    # SCUT_HEAD_Part_A/B --> JPEGImages: xxxx.jpg
    # sub_folders = ["SCUT_HEAD_Part_A", ]  # not suitable for accurate crowd detection
    sub_folders = ["SCUT_HEAD_Part_B", ]
    # sub_folders = ["SCUT_HEAD_Part_A", "SCUT_HEAD_Part_B]
    
    for sub_id, sub_folder in enumerate(sub_folders):
        
        '''update gt_dict'''
        gt_path = os.path.join(args.root_path, sub_folder, "Annotations")
        xmls_list = os.listdir(gt_path)
        for xml_file in xmls_list:
            img_name = str(sub_id) + "_" + xml_file[:-4] + ".jpg"  # to distinguish sub_folder
        
            xml_path = os.path.join(gt_path, xml_file)
            tree = ET.parse(xml_path)
            root = tree.getroot()
            objs = root.findall('object')
            for id, obj in enumerate(objs):  # only one object here
                # name = obj.find('name').text  # name should be one class "person" here
                ...
                gt_dict['width'].append(float(xmax - xmin))
                gt_dict['height'].append(float(ymax - ymin))
                gt_dict['ignore'].append(False)

        '''load images in sub_folder'''
        imgs_path = os.path.join(args.root_path, sub_folder, "JPEGImages")
